...
#     wp_in = list(map(int, waypoint.split()))
#     wp.append(wp_in)
# print(wp)
...
def compute_waypoints(mav):
    ...
    # if(check == 1):
        ...
        for i in range(n):
                wp_g.append(xy2latlon([x[i], y[i], z[i]], origin))

        for i in range(len(wps11)):
            wps11[i]=WP_Parameters()
    ...
    #         wps11[j].param3 = 0.0
    #         wps11[j].param4 = float('nan')
    #         wps11[j].x_lat = origin[0]
    #         wps11[j].y_long = origin[1]
    #         wps11[j].z_alt = origin[2]
